...
int insert_into_heap(heap *h, int val, int pri)
{
...
void percolate_down(heap *h, int i)
{
    ...
    if (l_child != -1 && h->p[l_child] < h->p[i])
        min_i = l_child;
    else
        min_i = i;

    if (r_child != -1 && h->p[r_child] < h->p[min_i])
        min_i = r_child;
...
void insert_element(heap *h, int data, int pri)
{
    if (!h || h->capacity == h->count)
        return;
...
void build_heap(heap *h, int *data_array, int *pri_array, int size)
{
    if (!h || !data_array || !pri_array || size > h->capacity || size <= 0)
        return;
...
int main ()
{
    heap *h = create_heap(5, 0);    //creating min heap
    int i = 0;
    int temp[5] = {0, 1, 2, 3, 4};
    int pri[5] = {4, 2, 1, 3, 6};

    build_heap(h, temp, pri, 5);
    cout << "After building heap: ";
    print_heap(h);

    delete_element(h, 0);
    cout << "After deleting 0th element: ";;
    print_heap(h);

    //insert element 10 with priority 5
    insert_element(h, 10, 5);
    cout << "After inserting 10 with pri 5: ";;
    print_heap(h);
